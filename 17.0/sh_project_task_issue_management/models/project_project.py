    ...
    def _compute_issue_count(self):
        for record in self:
            record.sh_issue_count = self.env['sh.issue'].search_count(
                [('sh_project_id', '=', self.id)])

    def _compute_project_issue_count(self):
        for record in self:
            record.sh_project_issue_count = self.env['sh.issue'].search_count(
                [('sh_project_id', '=', record.id)])

    sh_issue_count = fields.Integer(
        string="Issue Count", compute='_compute_issue_count',)

    sh_project_issue_count = fields.Integer(
        string="Project Issue Count", compute='_compute_project_issue_count',)

    def get_issues(self):
        self.ensure_one()
        issues = self.env['sh.issue'].sudo().search([
            ('sh_project_id', 'in', self.ids)
        ])
